...
class Point(object):
    ...
    def move_to(self, x, y):
        self.distance += self.get_distance_from(Point(x, y))
        self.x = x
        self.y = y
        self.move+= 1

    def move_by(self, dx, dy):
        self.distance += self.get_distance_from(Point(self.x+dx,self.y+dy))
        self.x+= dx
        self.y+= dy
        self.move+= 1
    ...
    def closer_to(self, p, p2):
        n = math.sqrt(((self.x-p.x)**2)+((self.y-p.y)**2))
        n2 = math.sqrt(((self.x-p2.x)**2)+((self.y-p2.y)**2))
